...
function StatusChangeModal({ aluno, novoStatus, consultores, onClose, onConfirm }) {
  ...
  const [form, setForm] = useState({
    descricao: '',
    consultor_id: aluno.consultor_id || '',
    data_fup: '',
  })
  ...
  async function confirmar() {
    ...
    await supabase.from('alunos').update({
      status: novoStatus,
      motivo_status: form.descricao.trim(),
      data_status_change: new Date().toISOString().split('T')[0],
    }).eq('id', aluno.id)

    // 2. Registrar no log de CS automaticamente
    await supabase.from('contatos_cs').insert({
      aluno_id: aluno.id,
      consultor_id: form.consultor_id || aluno.consultor_id || null,
      data_contato: new Date().toISOString().split('T')[0],
      churn_status_na_epoca: aluno.churn_cor || null,
      descricao: `[Status alterado para ${s.label}] ${form.descricao.trim()}`,
      data_fup: form.data_fup || null,
    })
  ...
  const isPausado = novoStatus === 'pausado'
  ...
  return (
    <Modal title={`Alterar status para ${s.label}`} onClose={onClose} width={500}>
      <div style={{background: isChurn ? '#fef2f2' : '#fef9c3', border: `1px solid ${isChurn ? '#fca5a5' : '#fde68a'}`, borderRadius:10, padding:'12px 14px'}}>
        <div style={{fontSize:13, fontWeight:600, color: isChurn ? '#991b1b' : '#713f12', marginBottom:4}}>
          {isChurn ? '⚠️ Atenção: esta ação marcará o aluno como Churn' : 'ℹ️ Aluno será marcado como Pausado'}
        </div>
        <div style={{fontSize:12, color: isChurn ? '#7f1d1d' : '#713f12', lineHeight:1.5}}>
          {isChurn
            ? 'O registro será salvo no log de CS. Descreva o motivo do churn e contexto relevante.'
            : 'O aluno ficará temporariamente inativo. Registre o motivo e defina um follow-up se necessário.'}
        </div>
      </div>
      ...
      <SelectField
        ...
        onChange={v=>setForm(f=>({...f,consultor_id:v}))}
        options={consultores.map(c=>({value:c.id,label:c.nome}))}
      />

      <Textarea
        label={isChurn ? 'Motivo do churn *' : 'Motivo da pausa *'}
        value={form.descricao}
        onChange={v=>setForm(f=>({...f,descricao:v}))}
        rows={4}
        required
        placeholder={isChurn
          ? 'Ex: Aluno não respondeu após múltiplas tentativas de contato. Solicitou reembolso...'
          : 'Ex: Aluno em viagem até março. Voltará a acompanhar as aulas em seguida...'}
      />

      {isPausado && (
        <Input
          label="Data de follow-up (quando retomar contato)"
          type="date"
          value={form.data_fup}
          onChange={v=>setForm(f=>({...f,data_fup:v}))}
        />
      )}
      ...
      <div style={{display:'flex',gap:8,justifyContent:'flex-end',paddingTop:4,borderTop:'1px solid var(--border)'}}>
        ...
        <Btn
          onClick={confirmar}
          variant={isChurn ? 'danger' : 'warning'}
          disabled={saving || !form.descricao.trim()}
        >
          {saving ? 'Salvando...' : `Confirmar ${s.label}`}
        </Btn>
      </div>
    </Modal>
  )
...
function StudentPanel({ aluno, aulas, reunioes, tarefas, presencas, notas, historico, contatos, consultores, turmas, onClose, onUpdate }) {
  ...
  const [statusModal, setStatusModal] = useState(null) // novoStatus a confirmar
  const [form, setForm] = useState({
  ...
  function handleStatusChange(novoStatus) {
    if (novoStatus === 'pausado' || novoStatus === 'churn') {
      setStatusModal(novoStatus)
      setForm(f=>({...f,status:aluno.status_aluno})) // reverte visualmente até confirmar
    } else {
  ...
  async function salvarContato() {
    ...
    await supabase.from('contatos_cs').insert({
      aluno_id:aluno.id, consultor_id:novoContato.consultor_id||aluno.consultor_id||null,
      data_contato:novoContato.data_contato, churn_status_na_epoca:novoContato.churn_status_na_epoca||null,
      descricao:novoContato.descricao.trim(), data_fup:novoContato.data_fup||null,
    })
  ...
  return (
    <>
        ...
        <div style={{width:500,height:'100vh',background:'var(--surface)',borderLeft:'1px solid var(--border)',overflowY:'auto',display:'flex',flexDirection:'column',animation:'slideIn 0.2s ease'}}>
          ...
          <div style={{padding:'18px 20px',borderBottom:'1px solid var(--border)',flexShrink:0}}>
            ...
            <div style={{display:'flex',gap:2,flexWrap:'wrap'}}>
              {tabs.map(t=>(
                <button key={t.id} onClick={()=>setTab(t.id)} style={{fontSize:12,fontWeight:tab===t.id?600:400,padding:'6px 12px',borderRadius:7,border:'none',background:tab===t.id?'var(--accent-dim)':'transparent',color:tab===t.id?'var(--accent-text)':'var(--muted)',cursor:'pointer',transition:'all 0.15s'}}>{t.label}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'18px 20px',display:'flex',flexDirection:'column',gap:16}}>
            ...
            {tab==='perfil'&&(<>
              {(aluno.churn_cor==='vermelho'||aluno.churn_cor==='roxo')&&(
                <div style={{background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:9,padding:'12px 14px'}}>
                  ...
                  <div style={{fontSize:12,color:'#7f1d1d',lineHeight:1.6}}>{aluno.churn_cor==='roxo'?'Matheus deve elaborar plano estratégico. Follow-up em 14 dias.':`${aluno.consultor_nome} deve agendar reunião de resgate. Follow-up em 14 dias.`}</div>
                </div>
              )}
              <div style={psec}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:7,marginTop:12}}>
                  {[{l:'U2A · perdeu 2 últimas aulas',v:aluno.u2a,p:'25%'},{l:'P3A · perdeu 3+ aulas',v:aluno.p3a,p:'25%'},{l:'Leads não preenchido',v:!aluno.leads_preenchido,p:'15%'},{l:'Financeiro não preenchido',v:!aluno.financeiro_preenchido,p:'15%'},{l:'Sem resposta aos fups',v:aluno.sem_resposta,p:'20%',full:true}].map((cr,i)=>(
                ...
                {editMode?(
                  <div style={{display:'flex',flexDirection:'column',gap:12}}>
                    ...
                    <div>
                      ...
                      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                        {['ativo','pausado','churn','inativo'].map(s=>{
                          const st=STATUS_ALUNO[s]
                          const isSelected=form.status===s
                          return(
                            <button key={s} onClick={()=>handleStatusChange(s)} style={{fontSize:12,fontWeight:600,padding:'6px 14px',borderRadius:8,cursor:'pointer',border:`2px solid ${isSelected?st.border:'var(--border)'}`,background:isSelected?st.bg:'var(--surface)',color:isSelected?st.text:'var(--muted)',transition:'all 0.15s'}}>
                              {st.label}
                            </button>
                          )
                        })}
                  ...
                  <div>
                    <table style={{width:'100%',fontSize:13,borderCollapse:'collapse',marginBottom:10}}>
                      <tbody>
                        {[['Especialidade',aluno.especialidade],['Clínica',aluno.clinica],['Cidade',aluno.cidade],['WhatsApp',aluno.whatsapp,'var(--accent-text)'],['Início',fd(aluno.data_entrada)],['Término previsto',fd(aluno.data_fim_prevista)],['Consultor',aluno.consultor_nome],['Turma',aluno.turma_nome],['Status',STATUS_ALUNO[aluno.status_aluno]?.label||'—']].map(([k,v,color])=>(
                          <tr key={k}><td style={{padding:'7px 0',color:'var(--muted)',borderBottom:'1px solid var(--border)',width:'45%'}}>{k}</td><td style={{padding:'7px 0',fontWeight:600,textAlign:'right',borderBottom:'1px solid var(--border)',color:color||'var(--text)'}}>{v||'—'}</td></tr>
                        ))}
                      </tbody>
                    </table>
                    <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                      ...
                      {!aluno.url_planilha_financeiro&&!aluno.url_planilha_leads&&<span style={{fontSize:12,color:'var(--muted)',fontStyle:'italic'}}>Nenhuma planilha vinculada · clique em Editar para adicionar</span>}
                    </div>
                  </div>
                )}
              </div>
              {historico.length>0&&(
                <div style={psec}>
                  <div style={{fontSize:10,fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10,fontFamily:'var(--mono)'}}>Histórico de status churn</div>
                  <div style={{paddingLeft:16,position:'relative'}}>
                    ...
                    {historico.map(h=>{const hc=cs(h.cor_nova);return(<div key={h.id} style={{position:'relative',marginBottom:12}}><div style={{position:'absolute',left:-14,top:4,width:8,height:8,borderRadius:'50%',background:hc.dot,border:`2px solid ${hc.border}`}}/><div style={{fontSize:10,color:'var(--muted)',fontFamily:'var(--mono)',marginBottom:2}}>{fd(h.criado_em?.split('T')[0])}</div><div style={{fontSize:12,color:'var(--text2)',lineHeight:1.4}}><strong style={{color:hc.text}}>{hc.label}</strong> · score {Math.round(h.score_novo)}%{h.cor_anterior&&<span style={{color:'var(--muted)'}}> (era {cs(h.cor_anterior).label})</span>}</div></div>)})}
                  </div>
                </div>
              )}
            </>)}
            ...
            {tab==='encontros'&&(
              <div>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                  ...
                  <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                    {Object.entries(PRESENCA_STATUS).map(([k,v])=>(<span key={k} style={{display:'flex',alignItems:'center',gap:4,fontSize:10,color:v.text}}><span style={{width:8,height:8,borderRadius:2,background:v.bg,border:`1px solid ${v.border}`,display:'inline-block'}}/>{v.label}</span>))}
                  </div>
                </div>
                ...
                {reunioes.length===0?(<div style={{textAlign:'center',padding:'24px 0',color:'var(--muted)',fontSize:13}}>Nenhuma reunião registrada</div>):reunioes.map(r=>(
                  <div key={r.id} style={{padding:'12px 14px',background:'var(--surface2)',borderRadius:9,marginBottom:8,border:'1px solid var(--border)'}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6,gap:8,flexWrap:'wrap'}}>
                      <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:12,fontFamily:'var(--mono)',fontWeight:500,color:'var(--text2)'}}>{fd(r.data_reuniao)}</span><span style={{fontSize:11,fontWeight:600,padding:'2px 8px',borderRadius:20,background:'var(--accent-dim)',color:'var(--accent-text)',border:'1px solid #bfdbfe'}}>{TIPOS_REUNIAO.find(t=>t.value===r.tipo)?.label||r.tipo}</span></div>
            ...
            {tab==='cs'&&(<>
              <div style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:10,padding:'14px 16px'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                    <SelectField label="Status na época" value={novoContato.churn_status_na_epoca} onChange={v=>setNovoContato(c=>({...c,churn_status_na_epoca:v}))} options={[{value:'verde',label:'Verde'},{value:'laranja',label:'Laranja'},{value:'vermelho',label:'Vermelho'},{value:'roxo',label:'Roxo'}]}/>
              ...
              <div>
                ...
                {contatos.length===0?(<div style={{textAlign:'center',padding:'24px 0',color:'var(--muted)',fontSize:13}}>Nenhum contato registrado</div>):contatos.map(ct=>{
                  ...
                  return(<div key={ct.id} style={{padding:'12px 14px',background:'var(--surface2)',borderRadius:9,marginBottom:8,border:'1px solid var(--border)'}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6,gap:8,flexWrap:'wrap'}}>
                      <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:12,fontFamily:'var(--mono)',fontWeight:500,color:'var(--text2)'}}>{fd(ct.data_contato)}</span>{ct.churn_status_na_epoca&&<span style={{fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20,background:hc.bg,color:hc.text,border:`1px solid ${hc.border}`}}>{hc.label}</span>}</div>
                      <div style={{display:'flex',alignItems:'center',gap:6}}>{ct.consultores?.nome&&<span style={{fontSize:11,padding:'2px 8px',borderRadius:20,background:'var(--surface2)',color:'var(--text2)',fontWeight:500,border:'1px solid var(--border)'}}>{ct.consultores.nome}</span>}{ct.data_fup&&<span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:20,background:'#fff7ed',color:'#7c2d12',border:'1px solid #fdba74'}}>FUP {fd(ct.data_fup)}</span>}</div>
                    </div>
            ...
            {tab==='notas'&&(<>
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                ...
                <div style={{display:'flex',justifyContent:'flex-end'}}><Btn onClick={salvarNota} variant='primary' disabled={savingNota||!novaNota.trim()}>{savingNota?'Salvando...':'Adicionar nota'}</Btn></div>
              </div>
              <div>
                ...
                {notas.length===0?(<div style={{textAlign:'center',padding:'24px 0',color:'var(--muted)',fontSize:13}}>Nenhuma nota</div>):notas.map(n=>(
                  <div key={n.id} style={{padding:'11px 13px',background:'var(--surface2)',borderRadius:9,marginBottom:8,border:'1px solid var(--border)'}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><span style={{fontSize:11,fontFamily:'var(--mono)',fontWeight:500,color:'var(--muted)'}}>{fd(n.created_at?.split('T')[0])}</span>{n.consultores?.nome&&<span style={{fontSize:11,padding:'2px 8px',borderRadius:20,background:'var(--accent-dim)',color:'var(--accent-text)',fontWeight:600}}>{n.consultores.nome}</span>}</div>
      ...
      {statusModal && (
        <StatusChangeModal
          aluno={aluno}
          novoStatus={statusModal}
          consultores={consultores}
          onClose={()=>setStatusModal(null)}
          onConfirm={()=>{ setStatusModal(null); setEditMode(false); onUpdate() }}
        />
      )}
    </>
  )
    ...
    <div style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
      ...
      <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
        {[{id:'',label:'Todas'},...turmas.map(t=>({id:t.id,label:`${t.nome} (${alunos.filter(a=>a.turma_id===t.id).length})`}))].map(t=>(
          <button key={t.id} onClick={()=>setFilterTurma(t.id)} style={{fontSize:12,fontWeight:500,padding:'5px 14px',borderRadius:20,border:`1.5px solid ${filterTurma===t.id?'var(--accent)':'var(--border2)'}`,background:filterTurma===t.id?'var(--accent)':'var(--surface)',color:filterTurma===t.id?'#fff':'var(--text2)',cursor:'pointer'}}>{t.label}</button>
        ))}
      </div>
      <div style={{marginBottom:16}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar aluno..." style={{fontSize:13,padding:'7px 12px',background:'var(--surface)',border:'1.5px solid var(--border2)',borderRadius:8,color:'var(--text)',outline:'none',width:220}}/>
      </div>
        ...
        {filtered.map(a=>{const cst=cs(a.churn_cor);return(
          <div key={a.id} onClick={()=>onSelectAluno(a.id)}
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
              ...
              <div><div style={{fontSize:13,fontWeight:600,color:'var(--text)',lineHeight:1.3}}>{a.nome}</div><div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>{a.turma_nome} · {a.consultor_nome}</div></div>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:10,paddingTop:10,borderTop:'1px solid var(--border)'}}>
              {[{label:'Aulas',val:`${a.aulas_presentes??0}/${a.total_aulas_realizadas??0}`},{label:'Reuniões',val:a.total_reunioes??0},{label:'Leads',val:a.leads_preenchido?'Sim':'Não',ok:a.leads_preenchido},{label:'Financeiro',val:a.financeiro_preenchido?'Sim':'Não',ok:a.financeiro_preenchido}].map(st=>(
                <div key={st.label} style={{fontSize:11,color:'var(--muted)'}}>
                  <strong style={{display:'block',fontSize:12,fontWeight:600,color:st.ok===false?'#dc2626':st.ok===true?'#15803d':'var(--text2)',fontFamily:'var(--mono)'}}>{st.val}</strong>{st.label}
                </div>
              ))}
            </div>
          </div>
      ...
      :view==='cards'?(
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(235px,1fr))',gap:10}}>
          {filtered.map(a=>{const cst=cs(a.churn_cor);return(
            <div key={a.id} onClick={()=>onSelectAluno(a.id)}
              ...
              <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}><Badge cor={a.churn_cor}/>{a.status_aluno!=='ativo'&&<StatusBadge status={a.status_aluno}/>}</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:10,paddingTop:10,borderTop:'1px solid var(--border)'}}>
                {[{label:'Especialidade',val:a.especialidade||'—'},{label:'Cidade',val:a.cidade||'—'},{label:'Reuniões',val:a.total_reunioes??0},{label:'Aulas',val:`${a.aulas_presentes??0}/${a.total_aulas_realizadas??0}`}].map(st=>(
                  <div key={st.label} style={{fontSize:11,color:'var(--muted)'}}><strong style={{display:'block',fontSize:12,fontWeight:600,color:'var(--text2)',fontFamily:'var(--mono)'}}>{st.val}</strong>{st.label}</div>
                ))}
              </div>
            </div>
          )})}
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            ...
            <tbody>
              {filtered.map(a=>(
                <tr key={a.id} onClick={()=>onSelectAluno(a.id)} onMouseEnter={e=>e.currentTarget.style.background='var(--surface2)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'} style={{borderBottom:'1px solid var(--border)',cursor:'pointer',transition:'background 0.1s'}}>
                  <td style={{padding:'11px 14px'}}><div style={{display:'flex',alignItems:'center',gap:9}}><div style={{width:30,height:30,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,flexShrink:0,background:cs(a.churn_cor).bg,color:cs(a.churn_cor).text,border:`1px solid ${cs(a.churn_cor).border}`}}>{ini(a.nome)}</div><div><div style={{fontWeight:600,color:'var(--text)'}}>{a.nome}</div><div style={{fontSize:11,color:'var(--muted)',marginTop:1}}>{a.cidade||'—'}</div></div></div></td>
...
function CalendarioModule({ turmas, aulas, alunos, presencas, onUpdate }) {
  ...
  const [editPresenca, setEditPresenca] = useState({}) // { aula_id: { aluno_id: status } }
  ...
  const presMap = {}
  presencas.filter(p=>turmaAlunos.some(a=>a.id===p.aluno_id)).forEach(p=>{
    if (!presMap[p.aula_id]) presMap[p.aula_id] = {}
    presMap[p.aula_id][p.aluno_id] = p.status
  })

  function getAulaStats(aulaId) {
    const ap = presMap[aulaId] || {}
    const presentes = Object.values(ap).filter(v=>v==='presente').length
    const total = turmaAlunos.length
    return { presentes, total, pct: total > 0 ? Math.round(presentes/total*100) : 0 }
  }

  function initEditPresenca(aulaId) {
    const ap = presMap[aulaId] || {}
    const init = {}
    turmaAlunos.forEach(a => { init[a.id] = ap[a.id] || null })
    setEditPresenca(init)
  ...
  async function salvarPresenca() {
    ...
    for (const [alunoId, status] of Object.entries(editPresenca)) {
      ...
      if (existing) {
        await supabase.from('presencas').update({status}).eq('id', existing.id)
      } else {
        await supabase.from('presencas').insert({aluno_id:alunoId, aula_id:selectedAula, status})
      }
    }
  ...
  async function marcarTodos(status) {
    const novo = {}
    turmaAlunos.forEach(a => { novo[a.id] = status })
    setEditPresenca(novo)
  }

  async function salvarAula() {
    if (!novaAula.tema.trim()||!novaAula.data_aula||!novaAula.numero) return
    setSavingAula(true)
    await supabase.from('aulas').insert({
      turma_id: selectedTurma,
      numero: parseInt(novaAula.numero),
      tema: novaAula.tema.trim(),
      data_aula: novaAula.data_aula,
      status: novaAula.status,
    })
    setNovaAula({numero:'',tema:'',data_aula:'',status:'futura'})
    setSavingAula(false)
  ...
  async function toggleAulaStatus(aulaId, currentStatus) {
    ...
    await supabase.from('aulas').update({status: next}).eq('id', aulaId)
    onUpdate()
  }

  const STATUS_AULA = {
    realizada: {bg:'#dcfce7',text:'#14532d',border:'#86efac',label:'Realizada'},
    proxima:   {bg:'#eff6ff',text:'#1d4ed8',border:'#bfdbfe',label:'Próxima'},
    futura:    {bg:'var(--surface2)',text:'var(--muted)',border:'var(--border)',label:'Futura'},
    cancelada: {bg:'#fef2f2',text:'#7f1d1d',border:'#fca5a5',label:'Cancelada'},
  }

  return (<>
    <div style={{padding:'13px 22px',background:'var(--surface)',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}>
      <div>
        <div style={{fontSize:16,fontWeight:700,color:'var(--text)'}}>Calendário</div>
        <div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>{turmaAulas.length} encontros · {turmaAlunos.length} alunos</div>
      </div>
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        <select value={selectedTurma} onChange={e=>{setSelectedTurma(e.target.value);setSelectedAula(null)}}
          style={{fontSize:12,padding:'6px 10px',background:'var(--surface)',border:'1.5px solid var(--border2)',borderRadius:7,color:'var(--text)',outline:'none',cursor:'pointer',fontWeight:500}}>
    ...
    <div style={{flex:1,overflowY:'auto',padding:'18px 22px',display:'grid',gridTemplateColumns:'minmax(0,1fr) 320px',gap:16,alignItems:'start'}}>
      ...
      <div>
        ...
        {turmaAulas.length===0?(
          <div style={{textAlign:'center',padding:'40px 0',color:'var(--muted)',fontSize:13}}>Nenhum encontro cadastrado</div>
        ):(
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {turmaAulas.map(au=>{
              ...
              return (
                <div key={au.id} style={{background:isSelected?'#eff6ff':'var(--surface)',border:`1px solid ${isSelected?'#bfdbfe':'var(--border)'}`,borderRadius:12,padding:'13px 16px',transition:'all 0.15s'}}>
                  <div style={{display:'flex',alignItems:'center',gap:12}}>
                    <div style={{width:36,height:36,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,fontFamily:'var(--mono)',flexShrink:0,background:sa.bg,color:sa.text,border:`1.5px solid ${sa.border}`}}>
                      {is1x1?'1:1':au.numero}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:3}}>
                        <div style={{fontSize:14,fontWeight:600,color:'var(--text)'}}>{au.tema}</div>
                        {is1x1&&<span style={{fontSize:10,fontWeight:600,padding:'1px 7px',borderRadius:20,background:'#ede9fe',color:'#2e1065',border:'1px solid #c4b5fd',flexShrink:0}}>1:1</span>}
                      </div>
                      <div style={{display:'flex',alignItems:'center',gap:10}}>
                        <span style={{fontSize:11,color:'var(--muted)'}}>{fdFull(au.data_aula)}</span>
                        {au.status==='realizada'&&(
                          <div style={{display:'flex',alignItems:'center',gap:6}}>
                            <div style={{width:60,height:4,background:'var(--surface2)',borderRadius:2,overflow:'hidden',border:'1px solid var(--border)'}}>
                              <div style={{height:'100%',background:pctColor,width:`${stats.pct}%`,borderRadius:2}}/>
                            </div>
                            <span style={{fontSize:10,fontWeight:600,color:pctColor,fontFamily:'var(--mono)'}}>{stats.pct}%</span>
                            <span style={{fontSize:10,color:'var(--muted)'}}>{stats.presentes}/{stats.total}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    <div style={{display:'flex',gap:6,flexShrink:0}}>
                      <button onClick={()=>toggleAulaStatus(au.id,au.status)} style={{fontSize:11,fontWeight:600,padding:'4px 10px',borderRadius:7,cursor:'pointer',background:sa.bg,color:sa.text,border:`1px solid ${sa.border}`,transition:'all 0.15s'}}>
                        {sa.label}
                      </button>
                      {au.status!=='futura'&&(
                        <Btn onClick={()=>isSelected?setSelectedAula(null):initEditPresenca(au.id)} variant={isSelected?'primary':'default'} style={{fontSize:11,padding:'4px 10px'}}>
                          {isSelected?'Fechar':'Presença'}
                        </Btn>
                      )}
                    </div>
                  </div>
      ...
      <div style={{position:'sticky',top:0}}>
        {selectedAula ? (()=>{
          ...
          return (
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
              <div style={{padding:'14px 16px',borderBottom:'1px solid var(--border)',background:'#eff6ff'}}>
                <div style={{fontSize:13,fontWeight:700,color:'var(--accent-text)'}}>{aula?.tema}</div>
                <div style={{fontSize:11,color:'#3b82f6',marginTop:2}}>{fd(aula?.data_aula)} · Registro de presença</div>
              </div>
              <div style={{padding:'12px 14px',borderBottom:'1px solid var(--border)',display:'flex',gap:6'}}>
                <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                  {['presente','faltou','reset','link'].map(s=>{const ps=PRESENCA_STATUS[s];return(
                    <button key={s} onClick={()=>marcarTodos(s)} style={{fontSize:11,fontWeight:600,padding:'4px 10px',borderRadius:7,cursor:'pointer',background:ps.bg,color:ps.text,border:`1px solid ${ps.border}`}}>
                      Todos: {ps.label}
                    </button>
                  )})}
                </div>
                ...
                {turmaAlunos.map(a=>{
                  ...
                  return(
                    <div key={a.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:'1px solid var(--border)'}}>
                      <div style={{width:28,height:28,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,flexShrink:0,background:cst.bg,color:cst.text,border:`1px solid ${cst.border}`}}>{ini(a.nome)}</div>
                      <div style={{flex:1,fontSize:12,fontWeight:500,color:'var(--text)',minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.nome}</div>
                      <div style={{display:'flex',gap:4,flexShrink:0}}>
                        {Object.entries(PRESENCA_STATUS).filter(([k])=>k!=='churn').map(([k,v])=>(
                          <button key={k} onClick={()=>setEditPresenca(ep=>({...ep,[a.id]:k}))} style={{width:24,height:24,borderRadius:5,border:`1.5px solid ${cur===k?v.border:'var(--border)'}`,background:cur===k?v.bg:'transparent',cursor:'pointer',fontSize:9,fontWeight:700,color:cur===k?v.text:'var(--muted)',transition:'all 0.1s'}}>
                            {k==='presente'?'P':k==='faltou'?'F':k==='reset'?'R':'L'}
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <div style={{padding:'12px 14px',display:'flex',gap:8,justifyContent:'space-between',alignItems:'center'}}>
                <span style={{fontSize:11,color:'var(--muted)'}}>
                  {Object.values(editPresenca).filter(v=>v==='presente').length} presentes · {Object.values(editPresenca).filter(v=>v==='faltou').length} faltaram
                </span>
                <Btn onClick={salvarPresenca} variant='primary' disabled={savingPresenca} style={{fontSize:12}}>
          ...
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'30px 20px',textAlign:'center'}}>
            <div style={{fontSize:13,color:'var(--muted)',marginBottom:6}}>Nenhum encontro selecionado</div>
            <div style={{fontSize:11,color:'var(--muted)'}}>Clique em "Presença" ao lado de um encontro para registrar</div>
          </div>
        )}
        ...
        {turmaAulas.some(a=>a.status==='realizada')&&(
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'16px',marginTop:12}}>
            <div style={{fontSize:11,fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:12,fontFamily:'var(--mono)'}}>Estatísticas da turma</div>
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {turmaAlunos.slice(0,8).map(a=>{
                ...
                return(
                  <div key={a.id} style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{width:22,height:22,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:8,fontWeight:700,flexShrink:0,background:cs(a.churn_cor).bg,color:cs(a.churn_cor).text,border:`1px solid ${cs(a.churn_cor).border}`}}>{ini(a.nome)}</div>
                    <div style={{flex:1,fontSize:11,color:'var(--text2)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.nome.split(' ')[0]} {a.nome.split(' ').slice(-1)[0]}</div>
                    <div style={{width:50,height:4,background:'var(--surface2)',borderRadius:2,overflow:'hidden',border:'1px solid var(--border)',flexShrink:0}}>
                      <div style={{height:'100%',background:pctColor,width:`${pct}%`,borderRadius:2}}/>
                    </div>
                    <span style={{fontSize:10,fontWeight:600,color:pctColor,fontFamily:'var(--mono)',minWidth:28,textAlign:'right'}}>{pct}%</span>
                  </div>
                )
              })}
    ...
    {/* Modal nova aula */}
    {showNovaAula&&(
      <Modal title="Novo encontro" onClose={()=>setShowNovaAula(false)} width={460}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          <Input label="Número" type="number" value={novaAula.numero} onChange={v=>setNovaAula(a=>({...a,numero:v}))} placeholder="Ex: 11" required/>
          <Input label="Data" type="date" value={novaAula.data_aula} onChange={v=>setNovaAula(a=>({...a,data_aula:v}))} required/>
          <div style={{gridColumn:'1/-1'}}><Input label="Tema / nome do encontro" value={novaAula.tema} onChange={v=>setNovaAula(a=>({...a,tema:v}))} placeholder="Ex: Aula 11, 1:1 - 5, Presencial..." required/></div>
          <div style={{gridColumn:'1/-1'}}>
            <SelectField label="Status inicial" value={novaAula.status} onChange={v=>setNovaAula(a=>({...a,status:v}))}
              options={[{value:'futura',label:'Futura'},{value:'proxima',label:'Próxima'},{value:'realizada',label:'Realizada'}]}/>
          </div>
        </div>
        <div style={{display:'flex',gap:8,justifyContent:'flex-end',paddingTop:4,borderTop:'1px solid var(--border)'}}>
          <Btn onClick={()=>setShowNovaAula(false)} variant='ghost'>Cancelar</Btn>
          <Btn onClick={salvarAula} variant='primary' disabled={savingAula||!novaAula.tema.trim()||!novaAula.data_aula||!novaAula.numero}>{savingAula?'Salvando...':'Adicionar encontro'}</Btn>
        </div>
      </Modal>
    )}
  </>)
